...
return new class extends Migration
{
    ...
    public function up(): void
    {
        ...
        if ($driver === 'mysql') {
            DB::statement("ALTER TABLE users MODIFY COLUMN role ENUM('super_admin', 'owner', 'aux', 'dye_stuff', 'ppic', 'operator', 'dashboard', 'fm', 'vp', 'kepala_regu', 'kepala_shift', 'spv_listrik', 'scm', 'adm_prod') DEFAULT 'dashboard'");
        } elseif ($driver === 'pgsql') {
            // Drop check constraint in PostgreSQL
            DB::statement("ALTER TABLE users DROP CONSTRAINT IF EXISTS users_role_check");
            // Re-add constraint with new value
            DB::statement("ALTER TABLE users ADD CONSTRAINT users_role_check CHECK (role::text = ANY (ARRAY['super_admin', 'owner', 'aux', 'dye_stuff', 'ppic', 'operator', 'dashboard', 'fm', 'vp', 'kepala_regu', 'kepala_shift', 'spv_listrik', 'scm', 'adm_prod']::text[]))");
        }
    }
