...
return new class extends Migration
{
    ...
    public function down(): void
    {
        $driver = DB::getDriverName();
        if ($driver === 'mysql') {
            DB::statement("ALTER TABLE approvals MODIFY action ENUM('move_machine', 'edit_cycle_time', 'delete_proses', 'create_reprocess', 'create_aux_reprocess', 'swap_position', 'topping_la', 'topping_aux')");
        } elseif ($driver === 'pgsql') {
            DB::statement("ALTER TABLE approvals DROP CONSTRAINT IF EXISTS approvals_action_check");
